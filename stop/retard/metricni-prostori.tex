...
\begin{definicija}
    ...
    \end{itemize}
    Množico $\Int A$ vseh notranjih točk množice $A$ imenujemo \df{notranjost} od $A$. 

    Množico $\Cl A$ vseh točk, za katere za vsak $r>0$, krogla $K(a, r)$ seka $A$, imenujemo \df{zaprtje} množice $A$.
    
    Množico $\partial A$ vseh robnih točk množice $A$ imenujemo \df{meja} množice $A$.     
\end{definicija}
