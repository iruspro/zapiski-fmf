...
\begin{definicija}
    Naj bo $K$ kolobar in naj bo $L \subseteq K, \ L \neq \emptyset$. Pravimo, da je $L$ \df{podkolobar} kolobarja $K$, če je $L$ na istih operacijah kolobar.
\end{definicija}

\begin{opomba}
    Podobno definiramo tudi \df{podalgebro} in \df{podpolje}.
\end{opomba}
...
\begin{definicija}
    Polje $E$ je \df{razšeritev} polja $F$, če je $F$ podpolje $E$.
\end{definicija}
...
\begin{definicija}
    Kolobarju $(\Z_n, +, \cdot)$ pravimo \df{kolobar ostankov po modulu $n$}.
\end{definicija}

\begin{definicija}
    Naj bo $K$ kolobar. Najmanjšemu naravnemu številu $n$, za katerega je $n \cdot 1 = \underbrace{1 + 1 + \ldots + 1}_n = 0$, pravimo \df{karakteristika} kolobarja $K$. Oznaka: $\ch K$. Če tak $n$ ne obstaja, pravimo, da ima kolobar $K$ karakteristiko $0$.
\end{definicija}
