...
\begin{definicija}
    Naj bo $S$ neprazna množica. \df{Operacija na množice $S$} je preslikava $*: S \times S \to S, \ (a,b) \mapsto a * b$.

    Operacija $*$ je \df{asociativna}, če $\all{a, b, c \in S} (a*b)*c = a*(b*c)$.

    Operacija $*$ je \df{komutativna}, če $\all{a, b \in S} a* b = b* a$.
\end{definicija}

\begin{definicija}
    Neprazna množica $S$ skupaj z operacijo $*$ je \df{polgrupa}, če je operacija $*$ asociativna.
\end{definicija}

\begin{definicija}
    Naj bo $S$ množica z operacijo $*$. Pravimo, da je $e \in S$ \df{enota (oz. nevtralni element) za operacijo~$*$}, če~$\all{x \in S} e * x = x * e = x$.
\end{definicija}
...
\begin{definicija}
    Polgrupa z enoto je \df{monoid}.
\end{definicija}

\begin{definicija}
    Naj bo $S$ množica z operacijo $*$ in $e \in S$ enota. Naj bo $x \in S$.
    \begin{itemize}
        \item Element $l \in S$ je \df{levi inverz} elementa $x$, če $l * x = e$.
        \item Element $d \in S$ je \df{desni inverz} elementa $x$, če $x * d = e$.
        \item Element $y \in S$ je \df{inverz} elementa $x$, če $x *y = y* x = e$.
    \end{itemize}
\end{definicija}
...
\begin{definicija}
    Pravimo, da je element $x \in S$ \df{obrnljiv}, če obstaja inverz od $x$.
\end{definicija}

\begin{definicija}
    Naj bo $S$ z operacijo $*$ monoid. Pravimo, da je $S$ \df{grupa}, če je vsak element iz $S$ obrnljiv.

    Če je operacija $*$ komutativna, pravimo, da je $S$ \df{Abelova grupa}.
\end{definicija}

V grupah ponavadi uporabljamo \df{miltiplikativni zapis}: operacija: $\cdot$, enota: $1$, inverz od $x$: $x^{-1}$, potenca: $x^n$.

V Abelovih grupah uporabljamo \df{aditivni zapis}: operacija: $+$, enota: $0$, inverz od $x$: $-x$, potenca: $nx$.
...
\begin{zgled}
    ...
    \begin{enumerate}
        ...
        \item Naj bo $X$ neprazna množica. Definiramo $\Sim(X) = \set{\text{vse bijektivne preslikave } f: X \to X}$. $(\Sim(X), \circ)$ je grupa, imenujemo jo \df{simetrična grupa} množice $X$.        
...
\begin{zgled}[Simetrije kvadrata]
    ...
    Kvadrat ima kvečjemu $8$ simetrij, ker je vsaka simetrija določena s sliko oglišča $1$ in informacijo, ali smo naredili zrcaljenje ali ne. Dobimo množico simetrij $D_{2 \cdot 4} = \set{\id, r, r^2, r^3, z, rz, r^2z, r^3z}$. $D_{2\cdot 4}$ je \df{diedrska grupa moči $8$}.
\end{zgled}
...
\begin{zgled}[Diedrska grupa moči $2n$]
    ...
    Množica vseh simetrij je $D_{2n} = \set{1, r, r^2, \ldots, r^{n-1}, z, rz, r^2zn, \ldots, r^{n-1}z}$. $D_{2n}$ je \df{diedrska grupa moči $2n$}.
\end{zgled}
...
\begin{primer}
    Naj bo $S = (\R^{n \times n}, \cdot)$, $S^* = \setb{A \in \R^{n \times n}}{\det A \neq 0} = \GL_n(\R)$. $\GL_n(\R)$ je \df{splošna linearna grupa $n \times n$ matrik}.
\end{primer}
...
\begin{definicija}
    Cikli dolžine $2$ so \df{transpozicije}.
\end{definicija}
...
\begin{definicija}
    Permutacija je \df{soda (oz. liha)}, če je produkt sodo (oz. liho) mnogo transpozicij.
\end{definicija}
...
\subsection{Podgrupe}
\begin{definicija}
    Naj bo $G$ grupa in $H \subseteq G, \ H \neq \emptyset$. $H$ je \df{podgrupa grupe $G$}, če je $H$ za isto operacijo tudi grupa. Oznaka $H \leq G$.
\end{definicija}
...
\begin{primer}
    ...
    \begin{enumerate}
        ...
        \item Definiramo $\SL_n(\R) = \setb{A \in \GL_n(\R)}{\det A = 1}$. Potem $\SL_n(\R) \leq \GL_n(\R)$. $\SL_n(\R)$ imenujemo \df{specialna linearna grupa}.
        \item Definiramo $\text{O}(n) = \setb{A \in \GL_n(\R)}{AA^T = A^TA = I}$. Potem $\text{O}(n) \leq \GL_n(\R)$.
        \item Definiramo $\text{SO}(n) = \setb{A \in \text{O}(n)}{\det A = 1}$. Potem $\text{SO}(n) \leq \text{O}(n)$. Grupo $\text{SO}(n)$ imenujemo \df{specialne ortogonalne matrike}.
    \end{enumerate}
\end{primer}
...
\begin{definicija}
    Naj bosta $H, K \leq G$. Definiramo $HK = \setb{hk}{h \in H, \ k \in K}$. Temu pravimo \df{produkt podgrup}.
\end{definicija}
...
\begin{definicija}
    Naj bo $H \leq G, \ a \in G$. Definiramo množico $aHa^{-1} = \setb{aha^{-1}}{h \in H}$. Potem $aHa^{-1} \leq G$. Temu~se~reče \df{konjungiranje podgrupe $H$ z elementom $a$}.
\end{definicija}
...
\begin{trditev}
    Naj bo $G$ grupa.
    \begin{enumerate}
        \item Definiramo $Z(G) = \setb{y \in G}{\all{x \in G} yx=xy}$. Potem $Z(G) \leq G$. Tej grupi pravimo \df{center grupe $G$}.
        \item Naj bo $a \in G$. Definiramo $C_G(a) = \setb{y \in G}{ya = ay}$. Potem $C_G(a) \leq G$. Tej podgrupi pravimo \df{centralizator elementa $a$ v $G$}.
    \end{enumerate}
\end{trditev}
...
\begin{definicija}
    Naj bo $G$ grupa, $H \leq H, \ a \in G$. \df{Ekvivalenčni razred elementa $a \in G$} je množica $[a] = \setb{b \in G}{a \sim b}$.
\end{definicija}
...
\begin{definicija}
    Množico $aH$ imenujemo \df{levi odsek grupe $G$ po podgrupi $H$}.
\end{definicija}

\begin{opomba}
    V grupo $G$ lahko vpeljamo tudi relacijo $\approx$ s predpisom $\all{a, b \in G} a \approx b :\liff ab^{-1} \in H$.

    To je ekvivalenčna relacija. Ekvivalentni razredi so $[a] = \setb{ha}{h \in H} =: Ha$, ki jih imenujemo \df{desni odseki}.
\end{opomba}

\begin{definicija}
    \df{Faktorska (oz. kvocientna) množica} glede na relacijo $\sim$ je množica $G/_\sim = \setb{aH}{a \in G} =: G/H$.
\end{definicija}
...
\begin{definicija}
    Naj bo $G$ končna grupa. Moč množce $G/H$ označimo z $G:H$ (oz $[G:H]$) in jo imenujemo \df{indeks podgrupe $H$ v grupi $G$}.
\end{definicija}
...
\begin{primer}
    ...
    Operacija $+$ na $\Z/{n\Z}$ je seštevanje po modulu $n$. Grupa $\Z_n := \Z/{n\Z}$ je \df{grupa ostankov po modulu $n$}, $|\Z_n| = n$.
\end{primer}
\begin{posledica}
...
\begin{definicija}
    Naj bo $G$ grupa in $X$ podmnožica v $G$. Potem označimo z $\gen{X}$ najmanjšo podgrupo v $G$, ki vsebuje množico $X$. To podgrupo imenujemo \df{podgrupa generirana z množico $X$}.
\end{definicija}
...
\begin{definicija}
    ...
    \begin{itemize}
        \item Če je $X \subseteq G$, za katero velja $G = \gen{X}$, pravimo, da je \df{$G$ generirana z množico $X$}. Elementam množice $X$ pravimo \df{generatorji grupe $G$}.
        
        Oznaka: Če je $X = \set{x_1, \ldots, x_n}$, pišemo $\gen{\set{x_1, \ldots, x_n}} = \gen{x_1, \ldots, x_n}$.
        \item Če je $G = \gen{x_1, \ldots, x_n}$, pravimo, da je \df{$G$ končno generirana grupa}.
        \item Če obstaja $x \in G$, da je $G = \gen{x}$, pravimo, da je \df{$G$ ciklična grupa}.
    \end{itemize}    
\end{definicija}
...
\begin{definicija}
    Naj bo $G$ grupa in $a \in G$. Najmanjšemu naravnemu številu $n$, za katerega velja $a^n=1$, pravimo \df{red}~elementa $a$. Če tak $n$ ne obstaja, pravimo, da ima $a$ neskončen red.
\end{definicija}
