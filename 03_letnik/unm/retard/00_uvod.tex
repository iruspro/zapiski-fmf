...
Numerična metoda je \emph{direktna}, če s točnim računanjem izračuna točno rešitev s končnim številom osnovnih operacij. Druga možnost so \emph{iterativne metode}, kjer je rešitev limita nekega konvergentnega zaporedja.
...
%
Množico vseh predstavljivih števil označimo s \(P(b, t, L, U)\).
...
\end{align*}
Pri standardu IEEE uporabimo \emph{zaokrožanje} in za \(\fl(x)\) izberemo predstavljivo število, ki je najbližje \(x\). Če pa \(x\) ravno na sredine, vzamemo tisto število, ki ima sodo zadnjo števko.
...
\begin{proof}
    Zapišemo število \(x\) v obliki 
    \[
        x = 0.d_1\ldots d_t + 0.0\ldots0d_{t+1}d_{t+2}\ldots
    \]
    in število \(\fl(x)\) v premični piki. Ocenimo napako pri zaokroževanju navzdol in navzgor.
\end{proof}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Če je funkcija \(f\) odvedljiva v točki \(x\), potem je 
        \[
            |D_n| \approx |f'(x)||x - \overline{x}|.
        \]
    \end{itemize}
...
Če se rezultat pri majhni spremembi argumentov (motnji oz.\ perturbaciji) ne spremeni veliko, je problem \emph{neobčutljiv}, sicer pa je \emph{občutljiv}. Občutljivost je povezana s samim problemom in neodvisna od numerične metode.
...
\begin{zgled}[Wilkinson]
    \todo
\end{zgled}

\subsection{Stabilnost numerične metode}
Stabilnost se navezuje na numerično metodo. Glavno orodje za preverjanje stabilnosti je \emph{analiza zaokrožitvenih napak}.
...
Če je za vsak \(x\) direktna napaka omejena s produktom občutljivosti in nekaj majhnega, je metoda \emph{direktno stabilna}. Obratno stabilna metoda je tudi direktno stabilna, obratno pa ni nujno res.
