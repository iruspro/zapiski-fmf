...
\begin{itemize}
    ...
    \begin{itemize}
        \item \(\omega_0 = \sqrt{\frac{k}{m}}\), \(\omega_0\) je \df{lastna frekvenca}
        \item \(t_0 = \frac{2 \pi}{\omega_0} = 2 \pi \sqrt{\frac{m}{k}}, \ \nu = \frac{1}{t_0}, \ \omega_0 = 2 \pi \nu\), \(t_0\) je \df{nihajni čas}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Zakoni upora:
    \begin{itemize}
        \item Linearni zakon: \(\vec{F} = C \vec{v}\). Ponavadi označimo \(\beta = \frac{C}{m}\) ali \(2 \beta = \frac{C}{m}\) in \(\beta\) imenujemo \df{koeficient dušenja}
    \end{itemize}
\end{itemize}
...
\]
%
kjer je \(F(t')\) \df{časovni potek motnje} in \(\sin(\omega_0 (t - t_i))\) \df{Greenova funkcija nedušenega nihala}.
%
...
\df{Newtonov zakon za \(i\)-ti utež} je
%
\[
...
\]
%
kjer \(\ds C^2 = \frac{kl^2}{m}\) \df{hitrost valovanja}.
