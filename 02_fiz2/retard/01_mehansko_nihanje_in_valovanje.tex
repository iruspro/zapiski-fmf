...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item[(3)] Sila upora (linearni zakon upora, viskoznost): \(\vec{F}_\text{u} = -C \vec{v} = -C \dot{y} \widehat{e}_y\), kjer je \(C > 0\) \df{koeficient upora}.
            \begin{itemize}
        ...
        \]
        \item Vpeljemo oznake: \(\beta := \frac{C}{m}, \ [\beta]  = s^{-1}\) je \df{koeficient dušenja}; in \( \omega_0^2 := \frac{k}{m}, \ [\omega_0^2] = s^{-2}\) je \df{lastna frekvenca}. Dobimo enačbo:
        \[
...
\begin{enumerate}
    \item Definiramo \(y' := y - y_0\) \df{odmik od ravnovesne lege}. S tem enačba postane homogena.
...
\begin{primer}
    ...
    Definiramo količine:
    \begin{itemize}
        \item \(t_0 = \frac{2pi}{\omega}\) je \df{nihajni čas} (čas enega nihaja).
        \item \(\nu = \frac{1}{t_0}\) je \df{frekvenca}.
        \begin{itemize}
...
\newpage
\textbf{(b) \(D = 0 \ (\omega = 0)\)}. V tem primeru dobimo \df{kritično dušenje}.
...
\textbf{(c) \(D > 0 \ (\omega^2 < 0)\)}. V tem primeru dobimo \df{nadkritično dušenje}. 
...
\]
%
\df{Newtonov zakon za \(i\)-ti utež} je torej
%
\[
...
\]
%
\(\ds \frac{\partial^2u(x,t)}{\partial x^2}\) imenujemo \df{drugi simetrični odvod} \(u\) po \(x\). Ta odvod je enak limiti na desni v izrazu \textcolor{red}{(*)}, če obstaja.
...
\]
%
kjer \(\ds C^2 = \frac{kl^2}{m}\). Tej enačbe pravimo \df{valovna enačba}.
\begin{itemize}
    \item \([C^2] = \frac{\text{m}^2}{\text{s}^2} \lthen [C] = \frac{\text{m}}{\text{s}}\). \(C\) je \df{hitrost valovanja}.
\end{itemize}
