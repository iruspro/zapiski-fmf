...
Danes bomo spoznali, katere množice imenujemo \df{fraktale}, kaj pomeni \df{fraktalna dimenzija}, katere vrste poznamo in po čem se razlikujejo. Izračunali bomo dimenzije različnih matematičnih in naravnih fraktalov, da bi bolje razumeli matematiko, ki se skriva v ozadju.
...
\begin{definicija}
    Naj bo \(A \subseteq \R^n, \ \delta > 0\). \df{\(\delta\)-okolcija \(A_\delta\) množice \(A\)} je 
...
\begin{definicija}
    ...
    \begin{itemize}
        \item \df{Vektorsko vsoto}: \(A + B = \setb{x +y}{x \in A \land y \in B}\);
        \item \df{Skalarno množenje}: \(\lambda A = \setb{\lambda x}{x \in A}\).
    \end{itemize}
\end{definicija}

\begin{definicija}
    Naj bo \(A \subseteq \R^n, \ A \neq \emptyset\). \df{Premer množice \(A\)} je 
...
\begin{definicija}
    Množica \(A \subseteq \R^n\) je \df{omejena}, če \(|A| < \infty\) (ekvivalentno: \(\some{\delta > 0} A \subseteq K(0, \delta)\)). 
\end{definicija}
...
\begin{definicija} Naj bosta \(X \subseteq \R^n\) in \(Y \subseteq \R^m\).
    \begin{itemize}
        \item Preslikava \(f: X \to Y\) je \df{Höldorjeva} stopnje \(\alpha > 0\), če
        \[\some{c > 0} \all{x, y \in X} |f(x) -f(y)| \leq c|x-y|^\alpha\]
        \item Če je \(\alpha = 1\), potem pravimo, da je preslikava \(f\) je \df{Lipschitzova}:
        \[\some{c > 0} \all{x, y \in X} |f(x) -f(y)| \leq c|x-y|\]
        \item Preslikava \(f: X \to Y\) je \df{bi-Lipschitzova}, če 
...
\begin{definicija} \ 
    \begin{itemize}
        ...
        \end{itemize}
        \item Najmanjšo \(\sigma\)-algebro na \(\R^n\), ki vsebuje vse odprte množice imenujemo \df{Borelova \(\sigma\)-algebra} in jo označimo z \(B(\R^n)\).
...
\begin{primer} Primeri mer:
    \begin{itemize}
        ...
        \begin{definicija}
            \df{Lebesgueva mera \(\mathcal{L}^n: \mathcal{P}(\R^n) \to [0, \infty]\)} je 
