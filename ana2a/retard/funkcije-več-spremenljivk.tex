...
\begin{definicija}
    Naj bo $D^\text{odp} \subseteq \RR^n$. \emph{Množico vseh $k$-krat zvezno parcialno odvedljivih funkcij} označimo z~$C^k(D)$. \emph{Množica gladkih funkcij} je $C^\infty(D) = \bigcap_{k=1}^\infty C^k(D)$. \emph{Množica zveznih funkcij na $D$} je $C(D)$.
\end{definicija}
...
\begin{opomba}
    Množica $C^k(D)$ z operacijama seštevanja, množenja s skalarji in komponiranja preslikav je algebra nad $\RR$.
\end{opomba}
...
\begin{lema}[Lagrangeev izrek za funkcijo več spremenljivk]
    Naj bo $D \subseteq \RR^n$ odprta množica, točki $a, b \in D$ taki, da za vsak $t \in [0,1]$ daljica $(1-t)a + tb \in D$, $f: D \to \RR$ funkcija razreda $C^1$.     
...
\begin{lema}
    Predpostavki kot prej. Naj obstaja tak $M \in \RR$, da za vsak $j = 1, \ldots, n$ in vsak $x \in D$ velja: $\left|\podv{f}{x_j}(x)\right| \leq M$. Tedaj~$|f(b)-f(a)| \leq M \sqrt{n}|b-a|$.
\end{lema}
...
\begin{lema}
    Naj bo $D \subseteq \RR^n, \ a,b \in D$ kot prej. Naj bo $F: D \subseteq \RR^n \to \RR^m, \ F = (f_1, \ldots, f_m)$ preslikava razreda~$C^1$. Naj obstaja tak $M \in \RR$, da za vsak $j = 1, \ldots, n$, vsak $i = 1, \ldots, m$ in vsak $x \in D$ velja: $\left|\podv{f_i}{x_j}(x)\right| \leq M$. 
...
Za vsak $x \in \RR^n$ naj bo $\podv{F}{y}$ diferencial preslikave $y \mapsto F(x,y)$. Imenujemo ga \emph{parcialni diferenical na drugo spremenljivko}. 
...
\begin{proof}
    ...
    Definiramo preslikavo $\Phi: D \subseteq \RR^n \times \RR^m \to  \RR^n \times \RR^m, \ \Phi(x,y) = (x, F(x,y))$. Kandidata za preslikavo $\varphi$ najdemo v oblike inverza $\Phi^{-1}$, nato enostavno preverimo lastnosti.
\end{proof}
...
\begin{zgled}
    Naj bo $x,y \in \RR, \ F(x, y) = x^2 + y^2 -1$. S pomočjo izreka o implicitni preslikavi pokaži, da v okolici točke $(0,1)$ rešitve enačbe $F(x,y) = 0$ graf neke preslikave $\varphi$. Določi tudi preslikavo $\varphi$.
\end{zgled}
