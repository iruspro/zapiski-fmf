...
\begin{definicija}
    Integrali tipa $\ds \int_{c}^{d} \left(\int_{a}^{b} f(x,y) \,dx \right) \,dy$ imenujemo \df{dvakratni integrali}. 
\end{definicija}
...
\begin{definicija}
    Integral \(\ds F(y) = \int_{a}^{\infty} f(x,y) \, dx\) \df{konvergira enakomerno} na \(Y\), če 
...
\begin{opomba}
    Zveznost (in odvedljivost) sta lokalni lastnosti (zvezna (oz. odvedljiva) v vsaki točki), tj. $f$ je zvezna na $Y$, če je zvezna v vsaki točki \(y \in Y\) (tudi, če je zvezna v okolici vsake točke \(y \in Y\)). Zato v prejšnji trditvi je za zveznost \(F\) na \(Y\) dovolj zahtevati, da je integral \df{lokalno enakomerno konvergira}, tj
...
\begin{definicija}
    Funkcija $\ds \Gamma (s) = \int_{0}^{\infty} x^{s-1}e^{-x} \, dx$ je \df{Eulerjeva funkcija gama}.
\end{definicija}
...
\begin{definicija}
    Funkcija \(\ds B(p,q) = \int_{0}^{1}x^{p-1}(1-x)^{q-1} \, dx\) je \df{Eulerjeva funkcija beta}.
\end{definicija}
