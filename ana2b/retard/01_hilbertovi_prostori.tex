...
\begin{definicija}
    \emph{Skalarni produkt} \(\scp{\ }{}\) je preslikava \(\scp{\ }{}: X \times X \to \R \text{ (oz. \(\C\))}\) za katero velja:
    \begin{enumerate}
...
\begin{opomba}
    1.\ -- 2.\ je \emph{pozitivna definitnost} skalarnega produkta, 3.\ je \emph{poševna simetričnost} (\emph{simetričnost} nad \(\R\)), 4.\ je linearnost v prvem faktorju.
\end{opomba}
...
\begin{definicija}
    \emph{Hilbertov prostor} je vektorski prostor \(X\) s skalarnim produktom \(\scp{\ }{}\), ki je v metriki, porojeni iz skalarnega produkta, poln metrični prostor.
\end{definicija}
...
\begin{opomba}
    \emph{Banachov prostor} je vektorski prostor \(X\) z normo \(\norm{\ }\), ki je v metriki, porojeni iz norme, poln metrični prostor.
\end{opomba}

\begin{zgled}
\
\begin{enumerate}
    \item Naj bo \(X = \R^n\). Definiramo skalarni produkt. Naj bo \(x, y \in \R^n, \ x = (x_1 \ldots, x_n), \ y = (y_1, \ldots, y_n)\). \emph{Standardni skalarni produkt} je \[x \cdot y = \sum_{k=1}^{n} x_ky_k.\]
    ...
    \item Naj bo \(X = \C^n\). Definiramo skalarni produkt. Naj bo \(z, w \in \C^n, \ z = (z_1 \ldots, z_n), \ w = (w_1, \ldots, w_n)\). \emph{Standardni skalarni produkt} je \[z \cdot w = \sum_{k=1}^{n} z_k\overline{w_k}.\]
...
\begin{definicija}
    Naj bo \((M, d)\) metrični prostor. Pravimo, da lahko \emph{napolnimo} prostor \(M\), če obstaja prostor \((\overline{M}, \overline{d})\), za kateri velja:
    \begin{enumerate}
    ...
    \end{enumerate}
    Prostoru \(\overline{M}\) rečemo \emph{napolnitev} prostora \(M\).
\end{definicija}
...
\begin{definicija}
    ...
    \begin{itemize}
        \item \(x\) je \emph{pravokoten} na \(y\), če \(\scp{x}{y} = 0\), tj. \(x \perp y \liff \scp{x}{y} = 0\).
        \item \emph{Ortogonalni komplement} množice \(A\) je \(A^\perp = \setb{x \in X}{\all{a \in A} x \perp a}\).
    \end{itemize}
\end{definicija}
...
\newpage
\begin{definicija}
    Naj bo \((X, \scp{\ }{})\) vektorski prostor s skalarnim produktom in \(Y \leq X\) podprostor X. Naj bo \(x \in X\). \emph{Pravokotna projekcija} vektorja \(x\) na podprostor \(Y\) (če obstaja) je tak vektor \(P_Y(x) \in Y\), da je 
    \[x - P_Y(x) \in Y^\perp.\]
\end{definicija}
...
\begin{definicija}
    ...
    \begin{itemize}
        \item Sistem vektorjev \((e_j)_{j=1}^\infty\) je \emph{ortogonalen sistem (ON)}, če \[\all{i,j \in \N} i \neq j \lthen \scp{e_i}{e_j} = 0.\]
        \item Tak sistem je \emph{ortonormiran (ONS)}, če \[\all{i,j \in \N} \scp{e_i}{e_j} = \delta_{ij}.\]
    \end{itemize}   
\end{definicija}
...
\begin{opomba} \
    \begin{itemize}
        ...
        \item \((\scp{x}{e_j})_{j = 1}^\infty\) so \emph{Fourierjevi koeficienti} \(x\) po ONS \(\sys\).
    \end{itemize}
\end{opomba}
