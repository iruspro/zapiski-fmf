...
\begin{definicija}
    ...
    \begin{itemize}
        \item Sistem vektorjev \((e_j)_{j=1}^\infty\) je \df{ortogonalen sistem (ON)}, če \[\all{i,j \in \N} i \neq j \lthen \scp{e_i}{e_j} = 0.\]
...
\begin{opomba} \
    \begin{itemize}
        ...
        \item \((\scp{x}{e_j})_{j = 1}^\infty\) so \df{Fourierjevi koeficienti} \(x\) po ONS \(\sys\).
    \end{itemize}
\end{opomba}
